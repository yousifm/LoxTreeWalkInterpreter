...
void PrinterVisitor::visitLiteral(std::shared_ptr<const Expr::LiteralExpr> expr) {
    _builder << std::visit(Token::variant_print(), expr->value());
}

void PrinterVisitor::visitGrouping(std::shared_ptr<const Expr::GroupingExpr> expr) {
